...
- [x] **部署到云服务器**（2026-09-24 实测）：线上为 **Docker 镜像路径** —— `/opt/garden-docker/docker-compose.prod.yml`，`network_mode: host` 直接接管 3000，连宿主**原生 PostgreSQL** `garden_db`（非 compose 的 postgres 服务），vault 以 bind mount 共享 `/opt/digital-garden/content/vault`；非 DEPLOY.md 主流程的 PM2 路径，原因：服务器仅 1.6G 内存，9-22 曾发生「watcher 全量重算叠加载荷拖垮整机」事故，现容器限 640M / Node 堆 384M，超限在容器内回收自愈。HTTPS 由 **acme.sh**（`/root/.acme.sh/liutianle.cn_ecc`）签发与续期，非 DEPLOY.md 写的 certbot；nginx `conf.d/garden.conf` 已含静态资源长缓存与 8m 上传上限（D5 配置行已生效）。当前镜像 2026-09-23 23:50 构建，含移动端全部改动与类型清偿（其后两个提交仅为文档与模板，不入产物）。**线上实测**：未登录 302 → `/login` 200、未登录 API 401；对 `https://liutianle.cn` 跑只读验收套件 `acceptance:shell` **47/47**、`acceptance:ui` **51/51**（底部 TabBar / FAB / 吸顶 / 长按面板 / 底部弹层 / 目录抽屉 / 密钥卡片在真机尺寸下全部通过）；容器内迁移 4/4 已应用、watcher 正常监听 vault。**遗留两步**：① Syncthing 双向同步未启用（无容器/服务，`/var/vault` 符号链接也未建）—— Obsidian → 线上秒级同步需按 DEPLOY.md §7 手动配对；② 种子初始密钥默认值 `'liutl'` 仍在源码（`server/plugins/seed-auth.ts`）且为线上真实 root 密钥 —— 建议在后台新建一个新 root 密钥、验证登录后删除种子密钥完成轮换
