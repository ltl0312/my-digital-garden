...
- [x] **阶段 M（移动端 22 屏落地）**（2026-09-23）：把 `design-preview` 的 22 屏移动设计稿落到真实前端（改造既有响应式，硬约束是不破坏 E4 已验收形态）。**M1** 断点单源：新增 `composables/useViewport.ts`（`BP.sm=640`/`BP.lg=1024` + 抽屉宽度公式 `min(320,max(300,视口×0.82))`），消除此前散落四处的断点字面量（E4 的 768–1023 抽屉打不开即源于 `md`/`lg` 混用）；`nuxt.config.ts` 显式声明 viewport 并加 `viewport-fit=cover`（否则 `env(safe-area-inset-bottom)` 恒为 0）。**M2** 移动端外壳：新增 `BottomTabBar`/`AppFab`/`CreateMenuItems`/`useNavItems`/`useScrollActivity` —— <640 底部 Pill TabBar（4 项，与图标栏同源）+ FAB（滚动隐藏，**向上滚动或停止 300ms 回归**）+ 顶栏吸顶与滚动联动分隔线 + 编辑态隐藏 TabBar；显隐一律走 CSS（`sm:hidden`）而非 JS 断点，避免手机首帧闪烁。**M3** 触屏手势：新增 `useLongPress`（仅 `pointerType==='touch'` / 500ms / 位移>8px 取消 / 触发后在 **window 捕获阶段**吞掉随后的 click —— 同元素监听器顺序不可靠）、`Sheet`/`ActionSheet`/`NoteActionSheet`、`useNoteActions`；列表行与结构树长按，桌面右键菜单与触屏底部面板**共用同一份菜单项与权限规则**。**M4** 浮层移动化：`AppDialog` 双形态（<640 贴底弹层 / ≥640 居中卡片，覆盖新建笔记/新建文件夹/重命名/删除确认）、`ImportDialog` 手机端全屏弹层、详情页顶栏插槽（`Teleport` 到 `#shell-header-actions`）承载「目录/更多」+ 目录改底部抽屉、图谱 <640 图例与统计收进底部折叠面板、`admin` 密钥表格在手机端卡片化（七列信息一项不减）
- [x] **M 验收（185 项全绿，均在 `pnpm build` 产物上执行）**：M2 移动端外壳 **47** · M3+M4 手势与浮层 **43** · E4 界面 **51** · 跨阶段 API **44** · 多页×亮暗×视口 **24 张**截图（0 条横向溢出 / 最小字号全部 12px / 亮暗底色 `rgb(245,244,239)` vs `rgb(10,13,19)` 确定性生效）；环境终态复位（密钥回到跑前 1 条、测试目录已清）
- [x] **M 顺带修复 3 个既有缺陷**：① **`TocRail` 容器为 `hidden xl:block` ⇒ <1280 时目录与属性在手机与平板上完全不可见**（真实功能丢失，现由顶栏目录按钮 + 底部抽屉承载，<1280 页内操作行另补「目录」入口）；② **普通用户在桌面端没有任何入口进入「我的密钥」**（`RailNav` 原为 `if (isAdmin) push('/admin')`，而 spec 8.4 规定该页所有角色可见）；③ **FAB 在图谱页压住统计数字** → 按 §5 ② 规范把启用范围收敛为 `FAB_ROUTES={'/','/notes'}`（顶栏「新建」仍在手机端保留，不构成入口死路）
- [x] **M 收口的方法性修正（3 类，全部记录以免重犯）**：① **E4 的 4 条断言重跑 FAIL** —— 2 条属**有意变更**（抽屉宽度 286→320 公式值；导入对话框 358→390 全屏弹层），2 条属**断言已失效（假通过）**（图谱「四角浮层不重叠」用「文本含按领域 + class 含 absolute」定位，M4 后匹配到了新的底部折叠面板容器而"碰巧通过"；导入对话框用 `.modal-x` + `[class*="rounded-overlay"]` 定位卡片，M4 改圆角写法后 `card=0` 连带三条全挂）→ 全部改用 `data-testid` / `data-dialog-card` 稳定钩子，并把断言改写为能反映新基线的形式；② **跨阶段 API 回归首次 34/44** —— 9 项 FAIL 同一根因：脚本硬编码的 user 密钥已被清理，8 项 user 断言拿 401 误报为越权失败 → 改为脚本自建临时密钥、终态按**跑前基线**核对（原为假设"应该是 2 条"）；③ **`typecheck` 原脚本是空跑（假绿）** —— `vue-tsc --noEmit` 对 solution-style 根 tsconfig **不构建引用项目**，`--listFiles` 输出 0 行、耗时 15s、退出码 0，这正是 execution-plan 里「vue-tsc 全项目跑通」长期未勾选的真因（不是跑不通，是**跑了个空**）→ 改为官方命令 `nuxt typecheck`；同时 `typescript` 由 `^7.0.2` 对齐到 `^5.9.3`（TS7 的 `exports` 不再暴露 `./lib/tsc`，而 `vue-tsc@3` 依赖该子路径）
- [x] **M 阶段类型债清算（可复现的基线对比法）**：以 `git checkout 4a64e5f9 -- app server` 取 M 阶段之前的基线 **92 条**，与改动后 **101 条**做集合对比（按「文件 + 错误码 + 消息」而非行号，避免行号偏移噪声）→ **M 阶段引入 9 条，已全部修掉**：`TS2554 Expected 0 arguments` × 8（`useLongPress` 的 `clear` 声明为 0 参，而模板绑定会传 `$event`；FileTree 6 + NoteRow 2）+ `TS2345 'TreeNode | undefined'` × 1（`promoted.value.root` 取自已开启 `noUncheckedIndexedAccess` 的数组下标，未判空）。复核 **92 = 基线 92**（零新增、零消失，纯修复）。**剩余 92 条为阶段 A–F 历史债**（`server/utils/markdown.ts` 30 · `FileTree.vue` 11 · `GraphView.vue` 8 · `verify.post.ts` 6 · `maturity.ts` 6 · 其余 31 条散在 15 个文件），全部是 strict + noUncheckedIndexedAccess 的严格性告警、**非运行期故障**，**本轮未修改**（避免把"移动端收口"扩成"全项目重构"，且批量加 `!` 断言会掩盖真实问题）→ `pnpm typecheck` 会如实报出，待单独立项
- [x] **部署到云服务器**（2026-09-24 实测）：线上为 **Docker 镜像路径** —— `/opt/garden-docker/docker-compose.prod.yml`，`network_mode: host` 直接接管 3000，连宿主**原生 PostgreSQL** `garden_db`（非 compose 的 postgres 服务），vault 以 bind mount 共享 `/opt/digital-garden/content/vault`；非 DEPLOY.md 主流程的 PM2 路径，原因：服务器仅 1.6G 内存，9-22 曾发生「watcher 全量重算叠加载荷拖垮整机」事故，现容器限 640M / Node 堆 384M，超限在容器内回收自愈。HTTPS 由 **acme.sh**（`/root/.acme.sh/liutianle.cn_ecc`）签发与续期，非 DEPLOY.md 写的 certbot；nginx `conf.d/garden.conf` 已含静态资源长缓存与 8m 上传上限（D5 配置行已生效）。**线上实测**：未登录 302 → `/login` 200、未登录 API 401；对 `https://liutianle.cn` 跑只读验收套件全部通过（见下条）
- [x] **v1.0.0 正式版发布**（2026-09-24）：① **种子密钥默认值移除**（`f45601a5`）—— 初始密钥只从 `SEED_ADMIN_KEY` 注入、仅密钥表为空时播种一次（废弃「按单键 findUnique 补种」—— 旧逻辑会让后台删除的种子密钥重启后复活），空库缺配置拒绝启动；源码与文档 `git grep liutl` 归零（API 示例 / PROJECT-* / 原型 mock 全部占位符化；**git 历史仍含该密钥，轮换仍需后台操作**）。② 服务器侧重建镜像 `my-digital-garden-app:v1.0.0`（`/opt/garden-src` docker build，约 5 分钟；**注意长 SSH 会话会被静默断开，构建命令的输出会随连接丢失 —— 须以「镜像时间戳 / 日志文件」核验结果，或 nohup 脱离会话**），容器已 recreate：迁移 4/4、watcher 正常、产物中 `liutl` 0 文件、`SEED_ADMIN_KEY` 就位、0 错误日志；线上只读验收 **shell 47/47 + ui 51/51**。③ 打 tag `v1.0.0` 并用 `gh release create` 发布（首个正式版）。④ **vault 同步决策**：Syncthing **不启用**（保持未安装/未配置），用户以自建 bat 上传/下载脚本同步 vault —— DEPLOY.md §7 与 compose 的 syncthing 服务对应步骤作废，`/var/vault` 符号链接无需创建
